#!/usr/bin/env python3
"""The Bonsai 2 pack loader: what it recognises, what it builds, and (with the pack) what it answers.
...
diff = float(mx.max(mx.abs(back.astype(mx.float32) - x.astype(mx.float32))).item())  # type: ignore[arg-type]
check("forward then inverse is the identity", diff < 0.05, f"max diff {diff:.4f}")
check("the transform preserves the norm (orthogonal)",
      abs(float(mx.sum(y.astype(mx.float32) ** 2).item()) / float(mx.sum(x.astype(mx.float32) ** 2).item()) - 1.0) < 0.02)  # type: ignore[arg-type]
try:
...
check("carries the format the kernels read", pl.bits == 2 and pl.group_size == 128 and pl.mode == "affine")
check("is frozen", not pl.trainable_parameters())
xin = mx.random.normal((2, 1024)).astype(mx.bfloat16)
want = mx.quantized_matmul(bl.fwht(xin, 1024, signs), q, s.astype(mx.bfloat16), b.astype(mx.bfloat16),
                           transpose=True, group_size=128, bits=2)
pl.set_dtype(mx.bfloat16)
got = pl(xin)
check("call is transform, then the affine matmul",
      float(mx.max(mx.abs(got.astype(mx.float32) - want.astype(mx.float32))).item()) < 0.05)  # type: ignore[arg-type]
plain = bl.PackedLinear((q, s, b), 0, None)
check("block 0 skips the transform",
      float(mx.max(mx.abs(plain(xin).astype(mx.float32)
                    - mx.quantized_matmul(xin, q, s, b, transpose=True, group_size=128, bits=2).astype(mx.float32))).item()) < 0.05)  # type: ignore[arg-type]
emb = bl.PackedEmbedding((q, s, b), 1024, signs, mx.bfloat16)
out = emb(mx.array([[3, 7], [0, 255]]))
check("the embedding returns [.., H] rows in the requested dtype", out.shape == (2, 2, 1024) and out.dtype == mx.bfloat16)
...
if "--with-models" in sys.argv[1:]:
    ...
    packed = [m for _, m in model.named_modules() if isinstance(m, bl.PackedLinear | bl.PackedEmbedding)]
    ...
    check("the embedding is packed", isinstance(model.language_model.model.embed_tokens, bl.PackedEmbedding))
    check("the model runs in bfloat16", model.language_model.model.norm.weight.dtype == mx.bfloat16)
    from mlx_lm import generate
    from mlx_lm.sample_utils import make_sampler

    prompt = tok.apply_chat_template([{"role": "user", "content": "What is 17*23? Answer with the number only."}],
                                     add_generation_prompt=True, tokenize=False, enable_thinking=False)
    text = generate(model, tok, prompt=prompt, max_tokens=8, sampler=make_sampler(0.0), verbose=False)
